# [HANDOFF] HANDOFF — VNV Linux Installer (automation guide)
...
| Component | State |
|---|---|
| **55 mods downloaded** + 5 extras, 0 HTML | [OK] (includes JAM 66666 + YUPDate/supplement 98514) |
| **55 mods imported** into MO2 | [OK] 50 active + 5 root (disabled, correct) + Fixed ESMs active |
| **Canonical VNV Core load order** | [OK] 23 plugins: base esms -> YUP -> d20Fixes -> UPNVSE+ -> NVMIM(-YUP) -> FaceGen -> Strip Lights -> LDF -> LTI(-YUP) -> fixy -> Placements -> JAM |
| **Root mods** | [OK] xNVSE (nvse_1_4 + steam_loader), 4GB (LAA=0x20 + import), BSAs vanilla (21 — decompressor NOT used, see BRAIN.md), UE ESM Fixes validated (465,054 records) |
| **Fixed ESMs (rebuild 7 aug)** | [OK] **465,054 records, DIALOG 18,215, INFO 23,247** (the old build had 233K records and 0 dialogues -> crash) |
| **INIs** | [OK] nvtf.ini (heap 400 + 4GB + VRAM, in-game and in mod), FalloutCustom.ini (Default profile) |
| **Guide extras** | [OK] JIP Settings INI, Stewie INI, JohnnyGuitar INI Presets, LOD Fixes INI, **JAM - VNV Configuration** (JustMods.ini) |
| **Game** | [OK] runs stable, 28 NVSE plugins loaded, new game playing without visible errors |
| **Error log** | [OK] 1.5 KB = only benign vanilla noise (rdt, misnamed BSA, optional texture swaps) |
| **JGNVSE "EDIDs conflicting"** | [OK] only 1 benign conflict (UPNVSE+ vs YUP: `UPNVSEPVendorQuestItemSCRIPT`) — the vanilla DLC conflicts disappeared with the correct Fixed ESMs |
...
- **`scripts/actualizar.py`** — updates `manifest.json` (names/versions/file_ids) choosing the most recent **MAIN** (`max(uploaded_timestamp)`). [WARN] CAUTION: if the mod has per-variant versions (e.g. "Placement Fixes TTW" vs "Placement Fixes"), the most recent MAIN may be the wrong variant -> verify masters (historical bug 90593 -> TTW).
- **`scripts/gestor_descargas.py`** — states in `estado.json` (pending/downloading/ok/failed), retries with backoff, automatic re-login. **Extras with `url` use direct download** (GitHub); extras with `file_id` go through Nexus. [WARN] Validate that `estado.json` points to the CORRECT file (there were cases where main<->extra got crossed: 58277 JIP dll, 84171 LOD INI).
