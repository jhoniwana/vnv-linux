...
correr_loot() {
  # VALIDACIÓN NO-DESTRUCTIVA: lootcli standalone NO ve el VFS de MO2, así que
  # descarta los esps de los mods y re-escribe plugins.txt con ~10 plugins si se
  # lo apunta al perfil real. Por eso se valida sobre una COPIA del plugins.txt
  # y el orden real de la guía (== orden LOOT) queda como está.
  ...
  cp -f "$MO2_INSTANCE/profiles/Default/plugins.txt" /tmp/opencode/loot_plugins.txt
  local dlls="Z:$(echo "$MO2_INSTANCE" | sed 's|/|\\\\|g')\\dlls"
  local game_path="Z:$(echo "$GAME_DIR" | sed 's|/|\\\\|g')"
  local plugin_list="Z:$(echo "/tmp/opencode/loot_plugins.txt" | sed 's|/|\\\\|g')"
...
lanzar() {
  ...
  if [[ -f "$prof/loadorder.txt" ]]; then
    if ! diff -q <(grep -v '^#' "$prof/loadorder.txt") <(grep -v '^#' "$prof/plugins.txt" | sed 's/^\*//') >/dev/null 2>&1; then
      ...
      {
        echo "# This file was automatically generated by Mod Organizer."
        grep -v '^#' "$prof/loadorder.txt" | sed 's/^/*/'
      } > "$prof/plugins.txt"
    fi
  fi
