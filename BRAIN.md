...
## ✅ CURRENT STATE — 100% VERIFIED (8 aug 2026)

| Component | State |
|---|---|
| Setup (setup.sh, multi-distro deps + venv + Camoufox + smoke test) | ✅ verified on Ubuntu 24.04, Arch, EndeavourOS |
| Nexus login (Turnstile auto-pass) | ✅ verified from scratch with real credentials (2026-08-08) |
| Downloads 55/55 (+5 extras) | ✅ verified live; fresh-clone continuity tested through 2 power-off interruptions |
| Install e2e (MO2 from scratch + 55 mods + root mods + INIs) | ✅ verified in `~/vnv-cero-test` (clean clone, fresh MO2 instance) |
| Root mods with post-step verification + fallbacks | ✅ xnvse/4gb/epic/uefix verified every run |
| BSAs + SArchiveList (21) | ✅ vanilla optimal; SArchiveList permanent in 3 INIs |
| Fixed ESMs (6/6 TES4, FalloutNV 465,054 records / 18,215 DIALOG) | ✅ validated; auto-inherit fallback when .mpi can't re-apply |
| `./vnv.sh salud` (12-point health check, exit code) | ✅ green on all machines |
| Web UI | ✅ 6/6 endpoint tests + SSE fix |
| Game | ✅ reaches main menu, stable 2+ min, no crashes, 28 NVSE plugins |
...
## 🚨 CRITICAL BUGS FOUND & FIXED (all verified)
...
## 🎯 BSA / DECOMPRESSOR — FINAL VERDICT (100% verified, 7-8 aug)
...
## ✅ THE DISTRIBUTED PIPELINE — WHAT RUNS TODAY
...
## 🔐 NEXUS LOGIN + DOWNLOADS (verified 8 aug, from scratch)
...
## 🧰 NATIVE LINUX PORTS (repos, all PUBLIC since 8 aug)

| Tool | Repo | Command | Useful? |
|---|---|---|---|
| xNVSE | `xnvse-linux` | root_mods step | ✅ essential |
| FNV 4GB Patcher | `fnv-4gb-patch-linux` | root_mods step | ✅ essential (LAA) |
| UE ESM Fixes | `ue-esm-fixes-linux` | `./vnv.sh esmfix` | ✅ essential (inherit fallback) |
| Epic Games Patcher | `epic-games-patcher-linux` | root_mods step | ⚠️ no-op on Steam (EGS only) |
| FNV BSA Decompressor | `fnv-bsa-decompressor-linux` | `./vnv.sh bsa` (research) | ❌ NOT needed (verdict above) |
...
## 🚧 KNOWN LIMITS / NOT TESTED (honest)
