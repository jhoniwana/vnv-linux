...
def importar(mo2_dir, args):
    ...
    # con --solo: preservar el loadorder/plugins del perfil (los demás mods ya
    # están importados y sus plugins deben seguir activos)
    if args.solo and (profile_dir / "loadorder.txt").exists():
        prev = (profile_dir / "loadorder.txt").read_text(errors="ignore").splitlines()
        if len(prev) > len(loadorder):
            for p in prev[1:]:
                p_ = p.strip("\r")
                if p_.lower() not in [x.lower() for x in loadorder]:
                    loadorder.append(p_)
