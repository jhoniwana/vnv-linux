#!/usr/bin/env python3
"""MASSIVE Nexus mod downloader (FREE account) — real /Download/ flow.

Discovery: https://www.nexusmods.com/Download/?id={file_id}&game_id=130&source=ModPage
shows the download page with a "Download" button (next to "served via CDN").
Click → downloads the real file. Works for free accounts.
...
def main():
    ...
    ap.add_argument("--seccion", help="only that manifest section")
    ap.add_argument("--solo", type=int, help="only that mod_id")
    ap.add_argument("--resume", action="store_true", help="skip already downloaded files")
    args = ap.parse_args()
    ...
    mods = [m for m in mods if m.get("file_id")]  # only those that have a file_id
    DEST.mkdir(exist_ok=True)

    print(f"🎯 {len(mods)} mods to download")
    ok, fail = 0, []
    ...
    with Camoufox(headless=True) as browser:
        ...
        def goto_y_challenge(url):
            """goto + wait if Cloudflare gives us 'Just a moment...'"""
            page.goto(url, timeout=90000, wait_until="domcontentloaded")
            page.wait_for_timeout(4000)
            for _ in range(12):  # up to 60s waiting for the challenge to pass
                try:
        ...
        for i, m in enumerate(mods, 1):
            ...
            # --resume: skip if any file of this mod already exists
            if args.resume:
                existentes = [p.name for p in DEST.iterdir() if str(m["mod_id"]) in p.name]
                if existentes:
                    print(f"[{i}/{len(mods)}] ⏭ already downloaded: {existentes[0][:60]}")
                    ok += 1
                    continue
            ...
            exito = False
            for intento in range(3):  # up to 3 attempts
                try:
                    if not goto_y_challenge(url):
                        raise RuntimeError("persistent Cloudflare challenge")
                    # accept the cookie consent (only the first time it appears)
                    if not consent_done:
                        try:
                            ...
                            pass
                    # click the Download button if it exists; otherwise the download starts by itself
                    dl_event = []
                    def on_dl(dl):
                        dl_event.append(dl)
                    page.on("download", on_dl)
                    # 1) wait for the auto-download (up to 12s)
                    for _ in range(12):
                        if dl_event:
                            break
                        page.wait_for_timeout(1000)
                    # 2) if it did not start, click the Download button (next to the CDN/automatically text)
                    if not dl_event:
                        res = page.evaluate("""() => {
                        ...
                        if res:
                            print("    → click on Download", flush=True)
                        for _ in range(40):  # up to 40s more
                            if dl_event:
                                break
                            page.wait_for_timeout(1000)
                    if not dl_event:
                    ...
                    if tam < 1000 and not d.suggested_filename.endswith((".7z", ".zip", ".rar")):
                        raise RuntimeError("suspiciously small/HTML download")
                    print(f"    ✔ {d.suggested_filename[:60]} ({tam//1024} KB)", flush=True)
                    ok += 1
                    exito = True
                    break
                except Exception as e:
                    print(f"    ✘ attempt {intento+1}: {type(e).__name__}: {str(e)[:90]}", flush=True)
                    time.sleep(15 * (intento + 1))  # backoff
            if not exito:
                fail.append((m["mod_id"], "3 failed attempts"))
            # human rate limit (8-15s)
